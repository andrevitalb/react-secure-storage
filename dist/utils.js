 ...
 * @returns
 */
const getSecurePrefix = () => {
  let KEY_PREFIX = _envHelper.default.getStoragePrefix() || "@secure.";
  if (!KEY_PREFIX.endsWith(".")) return KEY_PREFIX + ".";
  return KEY_PREFIX;
};
exports.getSecurePrefix = getSecurePrefix;
const FINGERPRINT_KEYS = exports.FINGERPRINT_KEYS = {
  USERAGENT: "UserAgent",
...
const getDisabledKeys = () => {
  let DISABLED_KEYS = _envHelper.default.getDisabledKeys() || "";
  ...
  DISABLED_KEYS.split("|").forEach(key => {
    if (key === "") {} else if (allOptions.includes(key)) response.push(key);else console.warn(`react-secure-storage : ${key} is not present in the available disabled keys options! Please go through the documentation`);
  });
  return response;
};
